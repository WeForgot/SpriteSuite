                ...
                if username == 'spriteclub':
                    if 'Betting open' in message:
                        ...
                        if match_array is None:
                            ...
                            continue
                        predictions = model(np.asarray([match_array]))
                        max_pred = tf.math.argmax(predictions[0]).numpy()
                        print('Bet {}'.format(Winner_Hint[max_pred]))
                        

if __name__ == '__main__':
    db_path = os.path.join('..','MatchScraper','sprite.db')
    model_path = os.path.join('..','NeuralClubbing','checkpoints','sprite_10_0_665')
